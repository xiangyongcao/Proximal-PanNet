...
test_data = './testdata/OriginalScale.mat'
#data = h5py.File(test_data)  # case 2: for large data (for real training v7.3 data in matlab)
data = sio.loadmat(test_data)

#### read training data ####
PAN = data['pan1'][...]  #### Pan image N*H*W
PAN = np.array(PAN,dtype = np.float32) /2047.

LMS = data['lms1'][...]  #### MS image interpolation -to Pan scale
...
Fused_result = np.zeros_like(LMS)
with tf.Session() as sess:
     ...
     sio.savemat('results_full_res_implicit.mat', {'Fused_result': Fused_result, 'pan': PAN, 'ms': MS, 'lms': LMS})
